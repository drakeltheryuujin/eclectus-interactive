...
gulp.task('eclectus:pixelate', function () {
    return gulp.src(tfDir + 'pixelated-images/**/*')
        .pipe(gulp.dest(tfDir + '/build/assets/images'));
});

gulp.task('eclectus:mailconfig', function () {
    return gulp.src(['mailer/config.php'])
        .pipe(replace(/(\$my_email = )'(.+)';/, '$1\'hello@eclectusinteractive.com\';'))
        .pipe(replace(/('username' => )'(.+)'/, '$1\'kellyecodes@gmail.com\''))
        .pipe(replace(/('password' => )'(.+)'/, '$1\'Mojojojo11\''))
